...
class JSONRPCResponse(BaseModel):
    ...
    success|error union); the SDK keeps its original name.
    """
...
# SDK error codes — SDK-internal allocations in the JSON-RPC
# implementation-defined server-error range [-32000, -32099]. Not defined by any
# MCP schema version. The spec now allocates into the same range
# (URL_ELICITATION_REQUIRED = -32042 in 2025-11-25;
# MISSING_REQUIRED_CLIENT_CAPABILITY = -32003 and
# UNSUPPORTED_PROTOCOL_VERSION = -32004 in 2026-07-28), so any future SDK-internal
# code must be chosen away from values the spec has allocated or is likely to
# allocate next.
CONNECTION_CLOSED = -32000
"""SDK-only error code: the connection closed before a response arrived.

Delivered to local request waiters when the session or dispatcher shuts down with
requests still pending; the SDK never emits this code on the wire.
"""

REQUEST_TIMEOUT = -32001
"""SDK-only error code: a request timed out while waiting for its response.

Raised locally as ``MCPError(code=REQUEST_TIMEOUT, ...)``.
"""

REQUEST_CANCELLED = -32002
"""SDK-only error code: an in-flight request was cancelled.

Written as a JSON-RPC error response when the dispatcher shuts down while the
...
(2024-11-05 through 2026-07-28). The SDK's error envelope is the generic
``ErrorData`` everywhere, with ``ErrorData.code`` carrying this value; the
2026-07-28 schema's per-code wrapper interfaces (``InternalError`` and its
siblings) are mirrored as data in ``mcp.types.v2026_07_28``, and nothing in
the SDK constructs them.
"""
...
class JSONRPCError(BaseModel):
    ...
    # If the 2025-11-25 optional-id reading is adopted, `id` gains a None default here.
    id: RequestId | None
    """The id of the request this error responds to.

    The member itself is required: ``None`` carries the JSON-RPC 2.0
    ``"id": null`` form (no request id could be determined, e.g. a parse
    error), and a frame missing the key entirely does not validate. The
    2025-11-25 and 2026-07-28 schemas can be read as making the member
    optional; the SDK keeps the required-but-nullable shape — which JSON-RPC
    2.0 and every earlier schema mandate — until the spec settles that
    reading. SDK-generated error responses always set the member.
    """
