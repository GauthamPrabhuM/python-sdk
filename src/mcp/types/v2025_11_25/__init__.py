...
and the one that serves every protocol version through 2025-11-25: the
schemas from 2024-11-05 to 2025-11-25 evolve strictly additively (each
defines a subset of the types and fields here), so the newest of them
describes them all. The wire-method maps point inbound validation for the
served versions at these models; emission is the plain monolith dump, so
this package's job is to be the reviewable record of the 2025-11-25 wire
shapes.

Initially generated from schema/2025-11-25/schema.json @ 6d441518de8a9d5adbab0b10a76a667a63f90665
...
class ElicitResult(WireModel):
    ...
    # legal wire values. The float arm follows schema.ts.
...
class NumberSchema(WireModel):
    ...
    # number fields too). The float arms follow schema.ts.
