...
protocol version, so application code handles a single set of types no
matter which version a session negotiated. Per-field docstrings record where
a field's availability differs across versions. The surface packages
(``mcp.types.v2025_11_25``, ``mcp.types.v2026_07_28``) carry the
schema-exact wire shapes; emission is the plain model dump at every
version. The newest schema covered here is
https://github.com/modelcontextprotocol/modelcontextprotocol/blob/main/schema/draft/schema.json.

Integration status: statements in this module that the session layer (or the
client session) is responsible for supplying or materializing the reserved
``io.modelcontextprotocol/*`` ``_meta`` entries on sessions negotiated at
2026-07-28 or later describe an intended integration contract, not behavior
the SDK has today. No session code supplies those entries yet, and the SDK
cannot yet negotiate 2026-07-28; ``mcp.types.methods`` carries the matching
status note for the wire-method maps.
"""
...
SDK-managed: the client session is responsible for supplying it on sessions
negotiated at 2026-07-28 or later. For the HTTP transport its value must
match the `MCP-Protocol-Version` header.
"""

CLIENT_INFO_META_KEY = "io.modelcontextprotocol/clientInfo"
"""Reserved request `_meta` key: the client `Implementation` making the request (2026-07-28).

SDK-managed: the client session is responsible for supplying it on every
request on sessions negotiated at 2026-07-28 or later.
"""
...
SDK-managed: the client session is responsible for supplying it on every
request on sessions negotiated at 2026-07-28 or later; servers must not
...
Replaces the former `logging/setLevel` RPC, and arrives already deprecated:
2026-07-28 deprecates the logging family as a whole (SEP-2577), so the same
revision that introduces this key also deprecates it. If absent, the server
must not send log notifications for this request.
"""
...
class RequestParamsMeta(TypedDict, extra_items=Any):
    ...
    state: the session layer is responsible for supplying all three reserved
    entries on every request it sends on sessions negotiated at 2026-07-28
    or later; inbound, the 2026-07-28-and-later surface types require them.
...
class RequestParams(MCPModel):
    ...
    Carries the optional progress token and, on sessions negotiated at
    2026-07-28 or later, the reserved `io.modelcontextprotocol/*` keys
    (protocolVersion, clientInfo, clientCapabilities, plus the deprecated
    logLevel). The field is required on the wire for 2026-07-28-and-later
    client requests: the session layer is responsible for supplying all
    three reserved entries when it sends a request; inbound, the
    2026-07-28-and-later surface types require them. Code sending requests
    through an SDK session normally leaves this field unset.
    """
...
class PaginatedRequest(Request[PaginatedRequestParams | None, MethodT], Generic[MethodT]):
    ...
    Required on the wire for 2026-07-28-and-later peers (because `_meta` is
    required there): the session layer is responsible for materializing
    ``params`` and the reserved `_meta` entries on sessions negotiated at
    2026-07-28 or later. Optional on all earlier versions.
    """
...
# The Literal arms document the spec-defined tags; the open str arm mirrors the
# schema's open union, so type checkers see plain str (no Literal narrowing).
...
class CacheableResult(Result):
    ...
    Both fields are required on the wire for 2026-07-28 peers and are always
    serialized by this SDK at every version (the defaults below are the
    SDK's choice; the schema declares no defaults). They do not exist in the
    2025-11-25 and earlier schemas; older peers ignore the extra keys.
    """
    ...
    0 (the default) means the response SHOULD be considered immediately
    stale; a positive value means the client SHOULD consider the result
    fresh for that many milliseconds. Must be non-negative.
    """
    ...
    With "public", any client or intermediary (e.g. a shared gateway or
    proxy) MAY cache the response and serve it to any user; with "private"
    (the default), only the requesting user's client MAY cache it, and
...
class EmptyResult(Result):
    ...
    `result_type` is None by default, so a freshly constructed EmptyResult
    dumps as `{}`. This is the one deliberate exception to the
    "results default to complete" rule: deployed TypeScript SDK clients
    (every released line, EmptyResultSchema.strict()) and the Rust SDK
    validate EMPTY results strictly and reject extra keys, so the SDK never
    volunteers `resultType` on an empty result. The 2026-07-28 schema makes
    `resultType` required on every result, empty ones included; code
    answering with an empty result on a session negotiated at 2026-07-28 or
    later (`mcp.shared.version.is_version_at_least`) must construct
    `EmptyResult(result_type="complete")` explicitly.
    """

    result_type: ResultType | None = None
    """None (the default) keeps the dump empty; see the class docstring."""
...
class DiscoverRequest(Request[RequestParams | None, Literal["server/discover"]]):
    ...
    ``io.modelcontextprotocol/*`` keys). The session layer is responsible
    for materializing ``params`` and supplying all three reserved entries,
    the same contract as for every other 2026-07-28 request; nothing is
    injected at dump time (emission is the plain model dump).
    """
...
class DiscoverResult(CacheableResult):
    ...
    Defaults to "complete" and is always serialized; pre-2026-07-28 peers
    ignore the extra key on non-empty results. Inbound bodies without the
    field (every pre-2026-07-28 peer) take the default, which the spec
    defines as equivalent to "complete". The union is open: a value outside
    the named literals (a future protocol's tag) is preserved as-is rather
    than rejected.
    """
...
2025-03-26 and 2025-06-18); serialization never narrows a value to fit --
tool blocks (and arrays of blocks) dump unchanged on 2025-06-18 and earlier,
just like audio content. Holding back a block type a peer's version predates
is the session layer's capability and version gating, not serialization's.
...
class ElicitRequestURLParams(RequestParams):
    ...
    New in protocol 2025-11-25: earlier schemas define no URL mode. Emission
    is the plain model dump at every version; sending URL mode only to peers
    that understand it is the session layer's capability and version gating,
    not serialization's.
    """
...
class ElicitResult(Result):
    ...
    The null value arm is monolith superset leniency: the surface packages
    match schema.ts, which declares no null arm, so an inbound null value
    fails the surface validation step.
    """
...
class InputRequiredResult(Result):
    ...
    """Tags this result as input-required (2026-07-28).

    The literal is the discriminating tag that lets the dual-result response
    unions (tools/call, prompts/get, resources/read) self-discriminate under
    pydantic smart-union validation. Defaulted so server handlers construct
    InputRequiredResult(...) without ceremony.
    """
...
# wire-method maps (mcp.types.methods), never enforced here -- inbound
# parsing stays superset-lenient on every session.
