...
One of the two schema-exact validator surfaces behind ``mcp.types.methods``:
the wire-method maps point inbound validation for 2026-07-28 sessions at
these models. The emitted bytes are always the plain monolith dump, never a
re-dump of a model here, and a monolith type with no counterpart in this
package has no 2026-07-28 wire form at all.

Initially generated from schema/draft/schema.json @ 6d441518de8a9d5adbab0b10a76a667a63f90665
(datamodel-code-generator 0.57.0), then hand-validated against the pinned
schema. Maintained as ordinary hand-edited source from here on; keep
definitions aligned with the pinned schema revision.

The models deliberately use ``extra="ignore"`` even where the schema declares
an object open to extra fields: unknown keys are accepted and dropped, and
the declared fields validate exactly — see ``mcp.types._wire_base`` for the
rationale. The classes kept open (``extra="allow"``) are commented in place.
Because unknown keys are ignored, a caller-set field this schema does not
define can never fail validation here: requiredness and value shapes are
what these models enforce.

At this revision the ``clientInfo``/``clientCapabilities`` values inside a
request's ``_meta`` validate through typed models, but vendor extra keys
nested inside those two values pass through emission: the reserved keys and
their values are caller data, never reshaped by the SDK.
...
# nested nulls must survive revalidation.
...
class ElicitResult(WireModel):
    """The result returned by the client for an `ElicitRequest` (`elicitation/create`) request."""
    ...
    # The float arm follows schema.ts.
...
class SubscriptionFilter(WireModel):
    ...
    """
    If true, receive `PromptListChangedNotification` (`notifications/prompts/list_changed`).
    """
    resource_subscriptions: Annotated[list[str] | None, Field(alias="resourceSubscriptions")] = None
    """
    Subscribe to `ResourceUpdatedNotification` (`notifications/resources/updated`) for these resource URIs.
    ...
    """
    If true, receive `ResourceListChangedNotification` (`notifications/resources/list_changed`).
    """
    tools_list_changed: Annotated[bool | None, Field(alias="toolsListChanged")] = None
    """
    If true, receive `ToolListChangedNotification` (`notifications/tools/list_changed`).
    """


class SubscriptionsAcknowledgedNotificationParams(WireModel):
    """Parameters for a `SubscriptionsAcknowledgedNotification`
...
class CompleteResult(WireModel):
    """The result returned by the server for a `CompleteRequest` (`completion/complete`) request."""
...
class CompleteResultResponse(WireModel):
    """A successful response from the server for a `CompleteRequest` (`completion/complete`) request."""
...
class ListRootsResult(WireModel):
    """The result returned by the client for a `ListRootsRequest` (`roots/list`) request.
...
class ProgressNotificationParams(WireModel):
    """Parameters for a `ProgressNotification` (`notifications/progress`) notification."""
...
class ReadResourceResult(WireModel):
    """The result returned by the server for a `ReadResourceRequest` (`resources/read`) request."""
...
class ResourceLink(WireModel):
    ...
    `ListResourcesRequest` (`resources/list`) requests.
    """
...
class SubscriptionsAcknowledgedNotification(WireModel):
    ...
    `SubscriptionsListenRequest` (`subscriptions/listen`) stream to acknowledge
...
class ListPromptsResultResponse(WireModel):
    """A successful response from the server for a `ListPromptsRequest` (`prompts/list`) request."""
...
class ListResourceTemplatesResultResponse(WireModel):
    """A successful response from the server for a `ListResourceTemplatesRequest` (`resources/templates/list`)
    request.
    """
...
class ListResourcesResult(WireModel):
    """The result returned by the server for a `ListResourcesRequest` (`resources/list`) request."""
...
class ListResourcesResultResponse(WireModel):
    """A successful response from the server for a `ListResourcesRequest` (`resources/list`) request."""
...
class ListToolsResult(WireModel):
    """The result returned by the server for a `ListToolsRequest` (`tools/list`) request."""
...
class ListToolsResultResponse(WireModel):
    """A successful response from the server for a `ListToolsRequest` (`tools/list`) request."""
...
class ToolResultContent(WireModel):
    ...
    If the tool defined a `Tool.outputSchema`, this SHOULD conform to that schema.
    """
...
class CallToolResult(WireModel):
    """The result returned by the server for a `CallToolRequest` (`tools/call`) request."""
...
class GetPromptResult(WireModel):
    """The result returned by the server for a `GetPromptRequest` (`prompts/get`) request."""
...
class CreateMessageResult(WireModel):
    """The result returned by the client for a `CreateMessageRequest` (`sampling/createMessage`) request.
...
class CallToolResultResponse(WireModel):
    """A successful response from the server for a `CallToolRequest` (`tools/call`) request."""
...
class DiscoverResult(WireModel):
    """The result returned by the server for a `DiscoverRequest` (`server/discover`) request."""
...
class DiscoverResultResponse(WireModel):
    """A successful response from the server for a `DiscoverRequest` (`server/discover`) request."""
...
class GetPromptResultResponse(WireModel):
    """A successful response from the server for a `GetPromptRequest` (`prompts/get`) request."""
...
class ReadResourceResultResponse(WireModel):
    """A successful response from the server for a `ReadResourceRequest` (`resources/read`) request."""
...
class RequestMetaObject(OpenWireModel):
    ...
    If absent, the server MUST NOT send any `LoggingMessageNotification` (`notifications/message`)
    ...
    `ProgressNotification` (`notifications/progress`)). The value of this parameter is an opaque token that will be
    attached to
...
class SubscriptionsListenRequestParams(WireModel):
    """Parameters for a `SubscriptionsListenRequest` (`subscriptions/listen`) request."""
...
"""Everything a tools/call response's result member may be at this version.

The same union the generated CallToolResultResponse.result field carries,
exported as a named alias so the wire-method maps (mcp.types.methods) can
reference it as a value.
"""

AnyGetPromptResult: TypeAlias = GetPromptResult | InputRequiredResult
"""Everything a prompts/get response's result member may be at this version."""

AnyReadResourceResult: TypeAlias = ReadResourceResult | InputRequiredResult
"""Everything a resources/read response's result member may be at this version."""
